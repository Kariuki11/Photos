...
export function ResultCard({ imageUrl, label, index }: ResultCardProps) {
  ...
  // ✅ Validate that imageUrl is a real string (not true, null, or object)
  const validImageUrl =
      ...
      >
        ...
        {validImageUrl ? (
          <img
            ...
            onError={() => setIsLoaded(true)} // in case image fails
          />
        ) : (
        ...
        {validImageUrl && (
          ...
          >
            ...
            <motion.button
              whileHover={{ scale: 1.1 }}
              whileTap={{ scale: 0.95 }}
              onClick={() => validImageUrl && window.open(validImageUrl, '_blank')}
              className="p-3 bg-white/90 text-gray-800 rounded-full hover:bg-white transition-colors"
            >
